-- Add 'on_order' as a valid status in purchase_orders table
DO $$ 
BEGIN
    ...
    -- Then add the new constraint with 'on_order' included
    ALTER TABLE purchase_orders 
    ADD CONSTRAINT purchase_orders_status_check 
    CHECK (status IN ('pending', 'submitted', 'approved', 'received', 'canceled', 'on_hold', 'on_order', 'rejected'));
    ...
    IF EXISTS (
        ...
        CHECK (approval_status IN ('pending', 'submitted', 'approved', 'received', 'canceled', 'on_hold', 'on_order', 'rejected'));
    END IF;
    ...
    IF EXISTS (
        ...
        -- Since we can't check column constraints easily in PostgreSQL, let's try to validate if we can insert an on_order value
        BEGIN
            ...
            WITH temp_update AS (
                ...
                WHERE approval_status = 'on_order' LIMIT 1
            )
            UPDATE purchase_orders 
            SET approval_status = COALESCE(status, 'pending')
            WHERE po_id IN (SELECT po_id FROM temp_update);
            
            RAISE NOTICE 'Successfully validated that approval_status accepts on_order value';
        EXCEPTION
            WHEN check_violation THEN
                ...
                RAISE NOTICE 'Fixed approval_status constraint to allow on_order status';
            WHEN OTHERS THEN
                RAISE NOTICE 'Unexpected error when testing approval_status constraints: %', SQLERRM;
        END;
    END IF;
    
    RAISE NOTICE 'Successfully updated purchase_orders status constraints to include on_order status';
END $$; 