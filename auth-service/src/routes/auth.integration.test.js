...
const EMAIL = 'integration@imms.test';
...
describe('auth integration', () => {
  ...
  it('POST /auth/login with unknown email → identical 401 envelope', async () => {
    const a = agent();
    const res = await a.post('/auth/login').send({ email: 'ghost@imms.test', password: 'whatever' });
    expect(res.status).toBe(401);
    expect(res.body.error).toBe('unauthorized');
    expect(res.body.message).toBe('Invalid credentials');
  });
...
describe('/admin/users integration', () => {
  it('login as admin → create user → list shows it', async () => {
    ...
    const create = await a.post('/admin/users').send({
      email: 'newbie@imms.test',
      display_name: 'Newbie',
      password: 'newbie-pw-1234',
      roles: ['mcs.tech'],
    });
    expect(create.status).toBe(201);
    expect(create.body.user.email).toBe('newbie@imms.test');
    expect(create.body.user.roles).toEqual(['mcs.tech']);

    const list = await a.get('/admin/users');
    expect(list.status).toBe(200);
    expect(list.body.users.find((u) => u.email === 'newbie@imms.test')).toBeTruthy();

    await pool.query(`DELETE FROM auth.users WHERE email = $1`, ['newbie@imms.test']);
  });

  it('non-admin role → 403 on /admin/users', async () => {
    await createTestUser(tenant.tenant_id, { email: 'viewer@imms.test', password: 'viewer-pw-1234', roles: ['mcs.viewer'] });
    const a = agent();
    await a.post('/auth/login').send({ email: 'viewer@imms.test', password: 'viewer-pw-1234' });
    const res = await a.get('/admin/users');
    expect(res.status).toBe(403);
    expect(res.body.error).toBe('forbidden');
    await pool.query(`DELETE FROM auth.users WHERE email = $1`, ['viewer@imms.test']);
  });
});
